...
function send()
{
    number1 = document.getElementById("number1_inp").value;
    number2 = document.getElementById("number2_inp").value;
    ans = parseInt(number1)*parseInt(number2);
    answer = number1*number2;

    que_number = "<h4>"+number1+" x "+number2+"</h4>";
    input_box = "<br>Ans:<input type='number' id='input_check_box'>";
    check_btn = "<br><br><button class='btn btn-info' onclick='check()'>Check</button>";
    row = que_number + input_box + check_btn;
    
    document.getElementById("output").innerHTML = row;

    document.getElementById("number1_inp").innerHTML = "";
    document.getElementById("number2_inp").innerHTML = "";
}
